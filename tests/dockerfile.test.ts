...
describe("Dockerfile", () => {
  ...
  it("bringt die native Abhaengigkeit ins Laufzeitabbild und prueft, dass sie laedt", () => {
    // Seit 2.11 braucht das Bundle SQLite mit Verschluesselung. Kopierte das
    // Abbild nur das Bundle, startete der Container nicht mehr.
    const laufzeit = dockerfile.slice(dockerfile.lastIndexOf("\nFROM "));
    expect(laufzeit).toContain("COPY npm/package.json");
    expect(laufzeit).toMatch(/npm install --omit=dev/);
    expect(laufzeit).toContain("require('better-sqlite3-multiple-ciphers')");
  });
