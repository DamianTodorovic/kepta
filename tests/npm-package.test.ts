...
describe("npm-Paket: das Bundle", () => {
  ...
  it("braucht ausser Node-Bordmitteln nur die SQLite-Verschluesselung", () => {
    // Seit 2.11 genau eine Abhaengigkeit: SQLite3 Multiple Ciphers. Zieht das
    // Bundle etwas anderes herein, fehlt es bei der Installation.
    const fremd = [...bundle.matchAll(/require\("([^"]+)"\)/g)]
      .map((m) => m[1])
      .filter((m) => !m.startsWith("node:"));
    expect([...new Set(fremd)]).toEqual(["better-sqlite3-multiple-ciphers"]);
  });
});
...
describe("npm-Paket: die package.json", () => {
  ...
  it("verlangt Node 22.13 — die gepruefte Untergrenze", () => {
    // Bis 2.10 lag hier node:sqlite zugrunde, das ohne Flag erst ab 22.13.0
    // laeuft. Seit 2.11 ist es SQLite3 Multiple Ciphers mit Vorab-Builds ab
    // Node 22; die Untergrenze bleibt, weil genau sie in CI geprueft wird.
    expect((paket.engines as Record<string, string>).node).toBe(">=22.13.0");
  });

  it("deklariert genau die eine Abhaengigkeit — exakt so gepinnt wie in der App", () => {
    const app = JSON.parse(fs.readFileSync(path.join(wurzel, "package.json"), "utf-8"));
    const version = app.dependencies["better-sqlite3-multiple-ciphers"];
    expect(version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(paket.dependencies).toEqual({ "better-sqlite3-multiple-ciphers": version });
  });
