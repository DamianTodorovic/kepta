...
/**
 * Der Schluessel der Datenbankdatei. Null = unverschluesselt oeffnen (Standard im
 * Kern und in den Tests; App und MCP-Server reichen den Schluesselbund herein,
 * siehe schluessel.ts).
 *
 * Synchron, weil der Store synchron oeffnet: der Schluessel muss VOR der ersten
 * Anweisung an die Datei gesetzt sein. Die fruehere asynchrone Fassung wurde nur
 * gefragt und nie abgewartet — ein Schluessel waere wirkungslos verpufft.
 */
export interface KeyProvider {
  /** Der 256-Bit-Schluessel fuer diese Datei, oder null fuer Klartext. Darf werfen. */
  keyFor(dbPath: string): Uint8Array | null;
  /** Wo der Schluessel liegt — fuer die Statusanzeige, nie der Schluessel selbst. */
...
export const PLAINTEXT_KEY: KeyProvider = { keyFor: () => null };
