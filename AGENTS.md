...
- **Session-Ende / wichtige Erkenntnisse**: `memory_save` — Entscheide selbst den Typ:
  - `semantic` = Fakten & Architektur (z. B. „KEPTA v2.0 nutzt node:sqlite, kein better-sqlite3")
...
## Projekt-Layout
- App: `ki-gehirn/` (React+Express+Electron, Server `server.ts`, Core `src/core/`, Tests `tests/`, Daten `~/.kepta/kepta.db`)
- Quality-Gates vor jedem Commit: `npx tsc --noEmit` + `npx vitest run` (42 Tests) — UX-Änderungen zusätzlich im Browser verifizieren.
