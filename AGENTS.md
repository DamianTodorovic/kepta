...
- Quality gates before every commit: `npx tsc --noEmit` (0 errors) + `npx vitest run` (325 tests) + `npm run build` (green, `dist` free of inline scripts) + `npm run eval` (Hit@1 ≥ 92 %) — verify UX changes in the browser as well.
